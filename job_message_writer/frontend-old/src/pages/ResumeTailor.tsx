...
const ResumeTailor: React.FC = () => {
  ...
  const handleCalculateInitialScore = async () => {
    ...
    try {
      ...
      console.log('Selected resume:', selectedResume.content);
      if (!selectedResume) {
  ...
  const handleOptimize = async () => {
    ...
    try {
      ...
      console.log('Optimization mera results:', response);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      {optimizationResults && (
        <div className="space-y-8">
          <ATSScoreDisplay
            originalScore={optimizationResults.original_ats_score}
            optimizedScore={optimizationResults.optimized_ats_score}
            breakdown={optimizationResults.optimized_sections ? {} : undefined}
            suggestions={[]}
          />

          {Object.entries(optimizationResults.optimized_sections || {}).map(([section, content]) => (
            <OptimizeSection
              key={section}
              title={section}
              originalContent={content.original_content}
              optimizedContent={content.optimized_content}
