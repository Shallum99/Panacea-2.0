...
function computeWordDiff(original: string, optimized: string): { text: string; type: "same" | "added" | "removed" }[] {
  ...
  while (i > 0 || j > 0) {
    if (i > 0 && j > 0 && origWords[i - 1] === optWords[j - 1]) {
      stack.push({ text: origWords[i - 1], type: "same" }); i--; j--;
    } else if (j > 0 && (i === 0 || dp[i][j - 1] >= dp[i - 1][j])) {
      stack.push({ text: optWords[j - 1], type: "added" }); j--;
    } else {
      stack.push({ text: origWords[i - 1], type: "removed" }); i--;
    }
  }
  stack.reverse();
...
// ── Build diff HTML document (renders in iframe like a PDF) ──
function escapeHtml(s: string) {
  return s.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/\n/g, "<br>");
}

function buildDiffHtml(changes: TextChange[]): string {
  const sections = new Map<string, TextChange[]>();
  for (const c of changes) {
    if (!sections.has(c.section)) sections.set(c.section, []);
    sections.get(c.section)!.push(c);
  }

  function wordDiffHtml(original: string, optimized: string, mode: "removed" | "added") {
    const diff = computeWordDiff(original, optimized);
    const parts = diff.filter(p => mode === "removed" ? p.type !== "added" : p.type !== "removed");
    return parts.map(p => {
      if (mode === "removed" && p.type === "removed")
        return `<span class="hl-del">${escapeHtml(p.text)}</span>`;
      if (mode === "added" && p.type === "added")
        return `<span class="hl-ins">${escapeHtml(p.text)}</span>`;
...
body{font-family:'Segoe UI',system-ui,-apple-system,sans-serif;background:#111827;color:#d1d5db;padding:24px 20px;font-size:10.5pt;line-height:1.7}
.section-hdr{font-size:9pt;font-weight:700;text-transform:uppercase;letter-spacing:2.5px;color:#6b7280;border-bottom:1px solid #1f2937;padding:16px 0 8px;margin-top:20px}
.section-hdr:first-child{margin-top:0}
.block{margin:10px 0;border-radius:6px;overflow:hidden;border:1px solid #1f2937}
.del-line{background:rgba(239,68,68,0.08);border-left:3px solid rgba(239,68,68,0.5);padding:10px 14px;color:#fca5a5;display:flex;gap:10px}
.ins-line{background:rgba(34,197,94,0.08);border-left:3px solid rgba(34,197,94,0.5);padding:10px 14px;color:#86efac;display:flex;gap:10px}
.prefix{color:#4b5563;font-weight:700;user-select:none;flex-shrink:0;font-family:'SF Mono',Consolas,monospace;width:14px;text-align:center}
.content{min-width:0;word-break:break-word}
.hl-del{background:rgba(239,68,68,0.25);color:#fecaca;border-radius:2px;padding:0 2px;text-decoration:line-through;text-decoration-color:rgba(239,68,68,0.5)}
.hl-ins{background:rgba(34,197,94,0.25);color:#bbf7d0;border-radius:2px;padding:0 2px;font-weight:500}
.tag{display:inline-block;font-size:8pt;padding:1px 8px;border-radius:10px;margin-left:10px;font-weight:500}
.tag-bullet{background:rgba(59,130,246,0.12);color:#93c5fd}
.tag-skill{background:rgba(168,85,247,0.12);color:#c4b5fd}
.tag-title{background:rgba(245,158,11,0.12);color:#fcd34d}
.sep{height:1px;background:#1f2937;margin:0}
.summary{text-align:center;padding:20px;color:#6b7280;font-size:9pt;border-top:1px solid #1f2937;margin-top:24px}
.summary strong{color:#d1d5db}
</style></head><body>`;

  let totalChanges = 0;
  for (const [section, sectionChanges] of sections) {
    html += `<div class="section-hdr">${escapeHtml(section)}</div>`;
    for (const c of sectionChanges) {
      totalChanges++;
      const tagClass = c.type === "bullet" ? "tag-bullet" : c.type === "skill" ? "tag-skill" : "tag-title";
      const tagLabel = c.type === "bullet" ? "bullet" : c.type === "skill" ? "skills" : "title";
      html += `<div class="block">`;
      html += `<div class="del-line"><span class="prefix">&minus;</span><div class="content">${wordDiffHtml(c.original, c.optimized, "removed")}<span class="tag ${tagClass}">${tagLabel}</span></div></div>`;
      html += `<div class="ins-line"><span class="prefix">+</span><div class="content">${wordDiffHtml(c.original, c.optimized, "added")}<span class="tag ${tagClass}">${tagLabel}</span></div></div>`;
      html += `</div>`;
    }
  }

  html += `<div class="summary"><strong>${totalChanges}</strong> change${totalChanges !== 1 ? "s" : ""} across <strong>${sections.size}</strong> section${sections.size !== 1 ? "s" : ""}</div>`;
  html += `</body></html>`;
  return html;
}
...
export default function GeneratePage() {
  ...
  // Generate diff HTML blob URL whenever tailorResult changes
  const diffBlobUrl = useMemo(() => {
    if (!tailorResult || tailorResult.changes.length === 0) return null;
    const html = buildDiffHtml(tailorResult.changes);
    const blob = new Blob([html], { type: "text/html" });
    return URL.createObjectURL(blob);
  }, [tailorResult]);

  // Cleanup diff blob on change
  useEffect(() => {
    return () => { if (diffBlobUrl) URL.revokeObjectURL(diffBlobUrl); };
  }, [diffBlobUrl]);
  ...
  const analyzeJd = useCallback(async (text: string) => {
    ...
    } catch { /* silent */ } finally { setAnalyzingJd(false); }
  }, []);
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      await streamApplication(
        ...
        (error) => { toast.error(error || "Failed to generate message"); setGenerating(false); },
        async () => {
          const supabase = createClient();
          const { data: { session } } = await supabase.auth.getSession();
  ...
  async function handleTailor() {
    ...
    try {
      ...
      } catch { /* PDF preview non-critical */ }
  ...
  async function handleSend() {
    if (!result) return;
    if (!editedRecipient.trim()) { toast.error("Enter a recipient email to send"); return; }
    setSending(true);
    try {
  ...
  const isAutoFilled = (field: string, value: string) => {
    ...
    const key = field === "positionTitle" ? "position_title"
      : field === "recruiterName" ? "recruiter_name" : "recipient_email";
  ...
  return (
    <div className="flex flex-col" style={{ height: "calc(100vh - 5rem)" }}>
      ...
      {/* Two-column split — full remaining height */}
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-0 flex-1 min-h-0">

        {/* ═══ LEFT COLUMN — Inputs ═══ */}
        <div className="xl:overflow-y-auto xl:pr-6 xl:border-r xl:border-border space-y-4 pb-6">
          {/* Resume selector */}
          <div>
            ...
            {loadingResumes ? (
              ...
              <p className="text-xs text-muted-foreground">
                No resumes uploaded.{" "}
                <a href="/resumes/upload" className="text-accent hover:underline">Upload one</a>
              </p>
            ) : (
              <select
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                value={positionTitle}
                onChange={(e) => { userEditedRef.current.add("positionTitle"); setPositionTitle(e.target.value); }}
            ...
            <div>
              ...
              <input
                value={recruiterName}
                onChange={(e) => { userEditedRef.current.add("recruiterName"); setRecruiterName(e.target.value); }}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                onChange={(e) => { userEditedRef.current.add("recipientEmail"); setRecipientEmail(e.target.value); }}
          ...
          <div className="flex-1 flex flex-col">
            <div className="flex items-center justify-between mb-1.5">
              ...
              <div className="flex items-center gap-3">
                {analyzingJd && (
                  <span className="text-[10px] text-accent animate-pulse">Analyzing JD...</span>
                )}
        ...
        {/* ═══ RIGHT COLUMN — Viewer ═══ */}
        <div className="xl:pl-6 flex flex-col min-h-0">
          {/* Tab bar */}
          {hasRightContent && (
          ...
          <div className="flex-1 min-h-0 overflow-hidden flex flex-col">
            ...
            {activeTab === "message" && (
              <div className="flex-1 overflow-y-auto">
                {generating ? (
                  streamedText ? (
                    <div className="border border-accent/30 rounded-b-lg overflow-hidden h-full flex flex-col">
                  ...
                  <div className="border border-border rounded-b-lg overflow-hidden h-full flex flex-col">
                    {/* Header */}
                    <div className="px-4 py-3 border-b border-border flex items-center justify-between shrink-0">
                      ...
                      <span className={`text-[10px] font-medium px-2 py-0.5 rounded ${
                        isSent ? "bg-green-500/10 text-green-500" : "bg-accent/10 text-accent"
                      }`}>
            ...
            {activeTab === "resume" && (
              <div className="flex-1 flex flex-col min-h-0">
                {tailoring ? (
                  <div className="flex-1 border border-border rounded-b-lg flex items-center justify-center">
                  ...
                  <div className="flex-1 flex flex-col min-h-0 border border-border rounded-b-lg overflow-hidden">
                    {/* ATS scores + view toggle bar */}
                    <div className="shrink-0 border-b border-border">
                      {/* ATS scores */}
                      <div className="grid grid-cols-2">
                        <div className="p-3 text-center border-r border-border">
                          <p className="text-xl font-bold">{Math.round(tailorResult.original_ats_score)}</p>
                          <p className="text-[10px] text-muted-foreground">Original ATS</p>
                        </div>
                        <div className="p-3 text-center bg-accent/5">
                          <p className="text-xl font-bold text-accent">{Math.round(tailorResult.optimized_ats_score)}</p>
                      ...
                      <div className="px-4 py-2 border-t border-border flex items-center gap-1">
                        {(["tailored", "original", "diff"] as const).map((view) => (
                          <button
                            key={view}
                            onClick={() => view === "original" ? handleViewOriginalPdf() : setPdfView(view)}
                    ...
                    <div className="flex-1 min-h-0 relative">
                      ...
                      {pdfView === "diff" && (
                        tailorResult.changes.length === 0 ? (
                          ...
                          <iframe src={diffBlobUrl} className="absolute inset-0 w-full h-full border-0" title="Resume Diff View" />
                        ) : null
                      )}
                    </div>
